...
# Custom chunking function using Ollama
def chunk_text_with_ollama(text):
    prompt = (
        "Please break this text into coherent chunks where each chunk contains similar meaning. "
        "Avoid mixing different topics or meanings in a single chunk.\n\n"
        f"Text:\n{text}"
    )
    response = ollama.run(model="llama3:8b", prompt=prompt)
    return response['chunks']  # Ensure this matches the expected output structure
...
def process_pdf(file_path, source_folder):
    ...
    # Chunk the document using Ollama
    print("Chunking document with Ollama...")
    start_time1 = time.time()
    chunks = chunk_text_with_ollama(full_text)
    end_time1 = time.time()
